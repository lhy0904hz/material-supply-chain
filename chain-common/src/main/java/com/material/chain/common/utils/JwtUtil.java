...
public class JwtUtil {
     ...
     * JWT 生成密钥使用的密码
     */
    private static final String JWT_RULE = "chain";
    ...
    private static SecretKey generateKey() {
        // 将密钥生成键转换为字节数组
        byte[] bytes = Base64.decodeBase64(JwtUtil.JWT_RULE);
        // 根据指定的加密方式，生成密钥
        return new SecretKeySpec(bytes, JwtUtil.JWT_ALG.getJcaName());
    }
    ...
    public static String buildJWT(String sub, String aud, String jti, String iss, Date nbf, Integer duration) {
        return buildJWT(JWT_ALG, generateKey(), sub, aud, jti, iss, nbf, duration);
    }
    ...
    public static String buildJWT(String sub, Integer duration){
        return JwtUtil.buildJWT(sub, null, UUID.randomUUID().toString(), null, null, duration);
    }
    ...
    public static Jws<Claims> parseJWT(String claimsJws){
        ...
        return JwtUtil.parseJWT(key, claimsJws);
    }
     ...
     */
    public static Boolean checkJWT(String claimsJws, String uri) {
        boolean flag = false;
        try {
            SecretKey key = generateKey();
            // 获取 JWT 的 payload 部分
            ...
            log.warn("JWT验证出错，错误原因：{},{}", e.getMessage(), uri);
            return false;
        }
        return flag;
    }
     ...
     * @return ture or false
     */
    public static Boolean checkJWT(Key key, String claimsJws, String sub, String uri) {
        boolean flag = false;
        try {
            // 获取 JWT 的 payload 部分
     ...
     * @return ture or false
     */
    public static Boolean checkJWT(String claimsJws, String sub, String uri) {
        return checkJWT(generateKey(), claimsJws, sub, uri);
    }
}